...
export class OrderService {
  ...
  parseDataToObject(data: any[][]) {
    ...
    for(let i = 1; i < data.length; i++) {
      this.getOrderFromRow(data[i], columnDict, shopeeOrders);
    }

    console.log(shopeeOrders);
  }

  getOrderFromRow(orderTableRow: any[], columnDict: { [ColumnName: string] : number }, shopeeOrders: ShopeeOrder[]) {
    ...
    var order = shopeeOrders.find(o => o.orderId === orderId);

    if(order) {
      if(order.orderStatus != OrderConstants.OrderStatusConstants.IS_CANCELED) {
